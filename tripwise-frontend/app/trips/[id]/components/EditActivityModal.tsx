...
export default function EditActivityModal({ tripId, item, isOpen, onClose, onSuccess }: any) {
    const [formData, setFormData] = useState({ activity: '', startTime: '', location: '' });

    useEffect(() => {
        if (item) {
          const formattedTime = item.startTime.substring(0, 16); 
          setFormData({ 
